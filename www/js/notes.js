...
function initNotes(userId) {
  notesRef = db.collection('users').doc(userId).collection('notes').doc('user_notes');
  ...
  if (savedColors) {
    try { customHighlightColors = JSON.parse(savedColors); } catch(e){}
  }
  renderColorPalette();
...
function toggleColorPalette(event) {
  ...
  const closeListener = (e) => {
    if (!e.target.closest('#color-palette-popover') && !e.target.closest('.toolbar-btn[title="Highlight"]')) {
...
function confirmCustomColor() {
  restoreSelection();
  if (pendingCustomColor && !customHighlightColors.includes(pendingCustomColor)) {
    customHighlightColors.push(pendingCustomColor);
    localStorage.setItem('liwanag_custom_colors', JSON.stringify(customHighlightColors));
  }
  if (pendingCustomColor) {
